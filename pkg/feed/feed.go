...
const (
	...
	// Maximum number of queued messages per connected client. Slow clients are
	// disconnected by the hub once this bounded queue fills.
	clientSendBuffer = 64
)
...
func serveWs(hub *Hub, w http.ResponseWriter, r *http.Request) {
	 ...
	 */
	client := &Client{hub: hub, conn: c, send: make(chan []byte, clientSendBuffer)}
...
func Handler(assetDir string) http.Handler {
	hub := newHub()
	go hub.run()
...
func Run(listenAddress, assetDir string) error {
	log.Printf("Olta Feed is available at http://%s/", listenAddress)
	return http.ListenAndServe(listenAddress, Handler(assetDir))
}
