...
func (p *Phishlet) LoadFromFile(site string, path string, customParams *map[string]string) error {
	...
	if !p.isVersionHigherEqual(&p.Version, "2.2.0") {
		return fmt.Errorf("this phishlet is incompatible with the current Olta Proxy version.\nplease do the following modifications to update it:\n\n" +
			...
			"you can find the phishlet 2.2.0 file format documentation here: https://github.com/s4l1hs/olta/wiki/Phishlet-File-Format-(2.2.0)")
	}
	if !p.isVersionHigherEqual(&p.Version, "2.3.0") {
		return fmt.Errorf("this phishlet is incompatible with the current Olta Proxy version.\nplease do the following modifications to update it:\n\n" +
			"- replace `landing_path` with `login` section\n" +
			"- change `min_ver` to at least `2.3.0`\n" +
			"you can find the phishlet 2.3.0 file format documentation here: https://github.com/s4l1hs/olta/wiki/Phishlet-File-Format-(2.3.0)")
	}
