...
func TestLoadConfigAppliesEmbeddedDatabaseDefaults(t *testing.T) {
	...
	if conf.DBName != DefaultDatabaseDriver {
		t.Fatalf("DBName = %q, want %q", conf.DBName, DefaultDatabaseDriver)
	}
	if conf.DBPath != DefaultDatabasePath {
		t.Fatalf("DBPath = %q, want %q", conf.DBPath, DefaultDatabasePath)
	}
}
