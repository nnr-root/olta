	...
	if len(msgs) > 0 {
		...
		for _, m := range msgs {
			...
			if len(rids) < 1 {
				...
				log.Infof("User '%s' reported email with subject '%s'. This is not an Olta campaign; you should investigate it.", m.Email.From, m.Email.Subject)
			}
			for rid := range rids {
				log.Infof("User '%s' reported email with rid %s", m.Email.From, rid)
				result, err := models.GetResult(rid)
				if err != nil {
					log.Error("Error reporting Olta campaign email with rid ", rid, ": ", err.Error())
					reportingFailed = append(reportingFailed, m.SeqNum)
					continue
				}
				err = result.HandleEmailReport(models.EventDetails{})
				if err != nil {
					log.Error("Error updating Olta campaign email with rid ", rid, ": ", err.Error())
					continue
				}
