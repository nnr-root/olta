...
func setupTest(t *testing.T) *testContext {
	...
	if err != nil {
		t.Fatalf("error getting test working directory: %v", err)
	}
	assetPath, err := filepath.Abs("../../../cmd/olta-campaign")
	if err != nil {
		t.Fatalf("error resolving campaign assets: %v", err)
	}
	if err := os.Chdir(assetPath); err != nil {
		t.Fatalf("error changing directories to campaign assets: %v", err)
	}
	conf := &config.Config{
		DBName: "sqlite3",
		DBPath: ":memory:",
