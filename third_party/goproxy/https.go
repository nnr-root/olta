	...
	case ConnectMitm:
		...
		go func() {
			...
			for !isEof(clientTlsReader) {
				...
				// OLTA PATCH: reproduce the origin's own framing instead of
				// forcing chunked transfer on every response. See
				// third_party/goproxy/OLTA-PATCHES.md.
				chunkResponseBody := applyOltaResponseFraming(resp)
				...
				} else if !chunkResponseBody {
					// OLTA PATCH: a response whose length is known is written
					// with that length, exactly as the origin framed it.
